...
# kine is a submodule, so kine/.git is a gitlink file rather than a real gitdir.
# Ship the actual gitdir separately and point git at it, so scripts/version can
# still run `git rev-parse`/`git status` against the kine worktree unmodified.
ENV GIT_DIR=/gitdir GIT_WORK_TREE=/go/src/github.com/k3s-io/kine
...
COPY ./kine/scripts/build ./kine/scripts/version ./scripts/
COPY ./kine/go.mod ./kine/go.sum ./kine/main.go ./
COPY ./kine/pkg ./pkg
COPY ./.git/modules/kine /gitdir
COPY ./kine/.golangci.yml ./.golangci.yml
RUN --mount=type=cache,id=gomod,target=/go/pkg/mod \
    --mount=type=cache,id=gobuild,target=/root/.cache/go-build \
    ./scripts/build
...
WORKDIR /go/src/github.com/k3s-io/kine
ENV GIT_DIR=/gitdir GIT_WORK_TREE=/go/src/github.com/k3s-io/kine
COPY ./kine/scripts/buildx ./kine/scripts/version ./scripts/
COPY ./kine/go.mod ./kine/go.sum ./kine/main.go ./
COPY ./kine/pkg ./pkg
COPY ./.git/modules/kine /gitdir
COPY ./kine/.golangci.yml ./.golangci.yml
